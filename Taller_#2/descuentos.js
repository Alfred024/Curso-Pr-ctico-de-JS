...
function calcularPrecioConDescuento(precioOriginal,descuento,cupon){
    return precioOriginal - (precioOriginal*((descuento+cupon)/100));
}
...
function calcularDescuentoButton(){
    ...
    const inputCuppon = document.getElementById("inputCuppon");
    const valueCuppon = inputCuppon.value;
    const resultado = calcularPrecioConDescuento(valuePrecio,valueDiscount,valueCuppon);

    const respuesta = document.getElementById("result");
    respuesta.innerText = "Precio con descuento y cupón: $"+resultado;
}
